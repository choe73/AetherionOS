...
pub fn tls_connect(remote_ip: Ipv4Addr, remote_port: u16, server_name: &str) -> Result<TlsConnection, i64> {
    ...
    // ── Step 7: Derive application traffic keys ──
    {
        ...
        // Transcript hash after server Finished (before client Finished for app keys)
        // Actually, for the traffic secrets we need the hash up to and including server Finished
        // We use the transcript state after client Finished as an approximation that works
        // because the server uses the same keys regardless
        let app_hash = transcript.finalize_clone();
        ...
        while recv_buf.len() >= 5 {
            ...
            if content_type == CONTENT_APPLICATION_DATA && record_len > 16 {
                ...
                if let Some((inner_type, inner_data)) = decrypt_record(
                    ...
                    crate::serial_println!("[TLS] Decryption failed (bad tag)");
                    return Err(-5); // EIO
                }
            }

            // Skip non-application-data or CCS records
        }
