...
/// Set extra arguments for the next ELF load (space-separated string)
pub fn set_extra_args(args: &str) {
    unsafe {
        ...
        for arg in args.split_whitespace() {
            ...
            ELF_EXTRA_ARGS[pos..pos+len].copy_from_slice(bytes);
    ...
    let (final_entry, final_rsp) = if let Some(ref interp_path) = result.interp_path {
        ...
        match interp_data {
            Ok(data) => {
                ...
                let argv = alloc::vec![alloc::string::String::from(path)];
                let envp = alloc::vec![
