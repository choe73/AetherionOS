    ...
    for (idx, component) in components.iter().enumerate() {
        ...
        if let Some(true) = is_symlink(current_ino) {
            if let Some(target) = read_symlink(current_ino) {
                ...
                let remaining: String = if idx + 1 < components.len() {
                    let rest: Vec<&str> = components[idx+1..].to_vec();
                    alloc::format!("{}/{}", target, rest.join("/"))
                } else {
                    target
                };
                // Recursively resolve (handles both absolute and relative symlinks)
                return lookup_path_follow(&remaining, depth + 1);
            }
